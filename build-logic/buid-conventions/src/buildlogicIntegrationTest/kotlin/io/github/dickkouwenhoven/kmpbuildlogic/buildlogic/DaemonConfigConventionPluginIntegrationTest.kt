...
class DaemonConfigConventionPluginIntegrationTest {
    @Test
    fun `daemon config plugin can be applied to project`() {
        val testProject = File("build/tmp/integrationTestDaemon").apply { mkdirs() }
        File(testProject, "build.gradle.kts").writeText(
            """
            plugins {
                id("kmpbuildlogic.daemon.config.plugin")
            }
            """.trimIndent(),
        )
